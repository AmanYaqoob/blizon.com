  ...
  (
    ...
    useEffect(() => {
      ...
      const handleMouseEnter = (e) => {
        if (e.target) e.target.style.textShadow = "0 0 20px rgba(203, 213, 225, 0.5)"
      }
    ...
    const pageStyles = `
      #mouse-gradient-react {
        ...
        background-image: radial-gradient(circle, rgba(22, 50, 91, 0.1), rgba(15, 20, 25, 0.05), transparent 70%);
      ...
      .grid-line { stroke: #16325B; stroke-width: 0.5; opacity: 0; stroke-dasharray: 5 5; stroke-dashoffset: 1000; animation: grid-draw 2s ease-out forwards; }
      .detail-dot { fill: #1A2947; opacity: 0; animation: pulse-glow 3s ease-in-out infinite; }
      .corner-element-animate { position: absolute; width: 40px; height: 40px; border: 1px solid rgba(26, 41, 71, 0.3); opacity: 0; animation: word-appear 1s ease-out forwards; }
      .text-decoration-animate { position: relative; }
      .text-decoration-animate::after { content: ''; position: absolute; bottom: -4px; left: 0; width: 0; height: 1px; background: linear-gradient(90deg, transparent, #1A2947, transparent); animation: underline-grow 2s ease-out forwards; animation-delay: 2s; }
      @keyframes underline-grow { to { width: 100%; } }
      .floating-element-animate { position: absolute; width: 2px; height: 2px; background: #1A2947; border-radius: 50%; opacity: 0; animation: float 4s ease-in-out infinite; animation-play-state: paused; }
      ...
      .ripple-effect { position: fixed; width: 4px; height: 4px; background: rgba(26, 41, 71, 0.6); border-radius: 50%; transform: translate(-50%, -50%); pointer-events: none; animation: pulse-glow 1s ease-out forwards; z-index: 9999; }
    `
    ...
    return (
      <>
        ...
        <section
          ref={ref}
          className={cn("relative z-0 flex w-full flex-col items-center justify-start overflow-hidden", className)}
        ...
        >
          ...
          >
            <defs>
              <pattern id="gridReactDarkResponsive" width="60" height="60" patternUnits="userSpaceOnUse">
                <path d="M 60 0 L 0 0 0 60" fill="none" stroke="rgba(22, 50, 91, 0.1)" strokeWidth="0.5" />
              </pattern>
            </defs>
          ...
          >
            <div className="absolute top-0 left-0 w-2 h-2 bg-[#1A2947] opacity-30 rounded-full"></div>
          </div>
          <div
            className="corner-element-animate top-4 right-4 sm:top-6 sm:right-6 md:top-8 md:right-8"
            style={{ animationDelay: "4.2s" }}
          >
            <div className="absolute top-0 right-0 w-2 h-2 bg-[#1A2947] opacity-30 rounded-full"></div>
          </div>
          <div
            className="corner-element-animate bottom-4 left-4 sm:bottom-6 sm:left-6 md:bottom-8 md:left-8"
            style={{ animationDelay: "4.4s" }}
          >
            <div className="absolute bottom-0 left-0 w-2 h-2 bg-[#1A2947] opacity-30 rounded-full"></div>
          </div>
          <div
            className="corner-element-animate bottom-4 right-4 sm:bottom-6 sm:right-6 md:bottom-8 md:right-8"
            style={{ animationDelay: "4.6s" }}
          >
            <div className="absolute bottom-0 right-0 w-2 h-2 bg-[#1A2947] opacity-30 rounded-full"></div>
          </div>
          ...
          <div className="relative z-10 min-h-screen flex flex-col justify-between items-center px-6 py-10 sm:px-8 sm:py-12 md:px-16 md:py-20 pt-24">
            ...
            <div className="text-center">
              ...
              <h2 className="text-xs sm:text-sm font-mono font-light text-slate-300 uppercase tracking-[0.2em] opacity-80 mb-8">
              ...
              >
                <Button
                  size="lg"
                  className="bg-[#1A2947] hover:bg-[#16325B] text-white px-8 py-3 text-lg font-semibold border border-white/20 hover:border-white/30 transition-all duration-300"
                  asChild
                >
